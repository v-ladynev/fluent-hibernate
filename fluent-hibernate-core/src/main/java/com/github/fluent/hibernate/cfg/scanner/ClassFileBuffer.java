...
/**
 * {@code ClassFileBuffer} is used to efficiently read Java ClassFile files from an
 * {@link InputStream} and parse the content via the {@link DataInput} interface.
 * <p>
...
final class ClassFileBuffer implements DataInput {
     ...
     * read pointer is reset to the start of the byte array.
     */
    ...
    public long readLong() throws IOException {
        ...
        return ((long) read() << 56) + ((long) read() << 48) + ((long) read() << 40)
                + ((long) read() << 32) + (read() << 24) + (read() << 16) + (read() << 8) + read();
    }
    ...
    /**
     * This methods throws an {@link UnsupportedOperationException} because the method is deprecated
     * and not used in the context of this implementation.
     *
