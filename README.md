...
Just pass `ImprovedNamingStrategyHibernate5` to the Hibernate 5 configuration. `ImprovedNamingStrategyHibernate5` implements the `ImplicitNamingStrategy` interface, so you can place `ImprovedNamingStrategyHibernate5` everywhere `ImplicitNamingStrategy` can be placed.
...
limitations can be encountered in rarily situations.

#### Constraint names generation
Foreign key and unique constraint names are generated by Hibernate 5 differ from ones are generated by Hibernate 4. 
I implement Hibernate 4 behaivour in `ImprovedNamingStrategyHibernate5`, but Hibernate 5  doesn't always use a naimg startegy to generate unique constraint names. For an, example: a unique constarin for `@OneToMany` annotation using a join table and  `@Column(unique = true)` dont't use `ImplicitNamingStrategy` to generate unique constrain names. You need to delete such unique constraint names yourself, before Hibernate 5 will update a schema.   


### A @DiscriminatorColumn name generation. 
